...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.warning("具体的に入力した方が、期待通りの回答を得やすいです")
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        for document in llm_response["context"][1:]:
            ...
            if "page" in document.metadata and sub_file_path.endswith(".pdf"): 
                # ページ番号を取得
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata and file_path.endswith(".pdf"):
                # ページ番号を取得
                page_number = document.metadata["page"]+1 # ページ番号は0スタートで取得されるため、1を加算して表示
                # 「ファイルパス」と「ページ番号」
